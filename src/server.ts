...
import { triggerOutbound } from "./voicelink/outbound";
...
export function buildApp(): Express {
  ...
  app.post("/webhooks/ghl", async function (req, res) { const token = typeof req.query.token === "string" ? req.query.token : ""; if (!safeEq(token, config.webhookToken)) { res.status(401).json({ error: "invalid token" }); return; } const b = req.body || {}; const to = String(b.phone ?? b.to ?? b.number ?? req.query.phone ?? "").replace(/[^0-9]/g, "").slice(-10); if (to.length < 3) { res.status(400).json({ error: "missing phone" }); return; } const agent = db.resolveAgent({}); const call = db.createCall({ direction: "outbound", to, status: "initiated", agentId: agent?.id, agentName: agent?.name, customParameters: { agent_id: agent?.id, source: "ghl" } }); bus.emitEvent({ type: "call.created", call }); try { const result = await triggerOutbound({ toNumber: to, agentId: agent?.id, callRef: call.id }); db.updateCall(call.id, { status: result.ok ? "ringing" : "failed", callSid: result.providerCallId ?? call.callSid, notes: result.ok ? undefined : ("lead api " + result.status + ": " + result.body.slice(0, 200)) }); res.status(result.ok ? 201 : 502).json({ ok: result.ok, callId: call.id, to }); } catch (err) { db.updateCall(call.id, { status: "failed", notes: (err as Error).message }); res.status(500).json({ error: (err as Error).message }); } });
  // --- Protected API ---
